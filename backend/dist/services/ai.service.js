...
async function extractTransactionsFromPDF(buffer, bankPassword) {
    ...
    if (isKTBStatement(text)) {
        ...
        if (ktbTxns.length > 0) {
            logger_1.logger.info(`KTB parser extracted ${ktbTxns.length} txns`);
            return ktbTxns;
        }
        logger_1.logger.warn('KTB statement detected but parser returned 0 txns — falling through to Ollama');
    }
    const CHUNK = 3000;
    const allExtracted = [];
...
${chunk}`;
    ...
    const sameBalance = (a, b) => a.balance != null && b.balance != null && Math.abs(a.balance - b.balance) < 0.01 &&
    ...
    for (const t of allExtracted) {
        if (!deduped.some(d => sameBalance(d, t)))
            deduped.push(t);
    }
    const fallback = parseTextFallback(text);
    if (deduped.length === 0) {
        logger_1.logger.info(`Ollama extracted 0 txns — using fallback (${fallback.length} txns)`);
        return fallback;
    }
    ...
    if (missed.length > 0) {
        logger_1.logger.info(`Fallback recovered ${missed.length} txns missed by Ollama`);
        deduped.push(...missed);
    }
    logger_1.logger.info(`Total extracted: ${deduped.length} txns (Ollama + fallback)`);
    return deduped;
}
...
function parseKTBStatement(text) {
    const results = [];
    ...
    for (const line of lines) {
        ...
        if (prevBalance !== null) {
            ...
            if (Math.abs(diff - amount) < 0.02)
                type = 'credit';
            else if (Math.abs(diff + amount) < 0.02)
                type = 'debit';
            else
